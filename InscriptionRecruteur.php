...
$errors=['prenom'=>'','nom'=>'','user'=>'' , 'nom_entreprise' => '' ,'email'=>'','phone'=>'','password'=>'' , 'confirm_password'=> '' ] ;

$prenom =  $nom =$user = $email= $nom_entreprise  = $phone = $password = $confirm_password =  '' ;
  ...
  $confirm_password = $_POST['confirm_password'];
    ...
    if (empty($_POST["prenom"])) {
     ...
     if (empty($_POST["nom"])) {
      ...
      // validation de nom_entreprise
      if (empty($_POST["nom_entreprise"])) {
        $errors['nom_entreprise'] = "Veuillez saisir votre nom de l'entreprise.";
      } else {
        $user =$_POST["nom_entreprise"];
        // Vérifier si le nom_entreprise ne contient que des lettres, des chiffres et des tirets bas
        if (!preg_match("/^[a-zA-Z0-9_]*$/",$nom_entreprise)) {
            $errors['nom_entreprise'] = "Seules les lettres, les chiffres et les tirets bas sont autorisés.";
        }
      }
      ...
      if(!array_filter($errors)) {
        $sql = "insert into inscrire_recruteur (prenom, nom, user, nom_entreprise, email, phone, passwordd)
        values (:prenom, :nom, :user, :nom_entreprise, :email, :phone, :password)";
$stmt = $pdo->prepare($sql);
$stmt->execute(['prenom' => $prenom, 'nom' => $nom, 'user' => $user, 'nom_entreprise' => $nom_entreprise, 'email' => $email, 'phone' => $phone, 'password' => $password]);
...
<body>
    ...
    <div class="register-photo">
        <div class="form-container">
            <div class="image-holder" style="background-image: url('images/cc.jpg');"></div>
            <form method="post">
                ...
                <div class="form-group">
                    <input class="form-control" type="text" name="user" placeholder="Nom d'utilisateur"  value="<?php echo htmlspecialchars($user); ?>" >
                    <div class="err">
                        <?php echo $errors['user'] ; ?>
                    </div>
                </div>
                <div class="form-group">
                    <input class="form-control" type="text" name="nom_entreprise" placeholder="Nom de l'entreprise"  value="<?php echo htmlspecialchars($nom_entreprise); ?>" >
                    <div class="err">
                ...
                <a href="ConnexionCondidat.html" class="already">Vous êtes déjà inscrit ? Connectez-vous ici !</a>
            </form>
        </div>
    </div>
